...
public:
     ...
     * TRACE,DEBUG用于调试，
     * INFO,WARN,ERROR,FATAL在程序运行过程中输出，警告的程度依次上升，
    ...
    static LogLevel logLevel(); // 返回当前的日志级别
    static setLogLevel(LogLevel level); // 设置日志级别

    typedef void (*OutputFunc)(consg char* msg, int len);
    typedef void (*FlushFunc)();
    static void setOutput(OutputFunc);  // 设置输出函数，用来替代默认的
    static void setFlush(FlushFunc);    // 用来设置你设置的输出函数的刷新方法
...
private:
    ...
    public:
        ...
        void finish();
        /**
         * formatTime函数的作用是将出错的时间格式化问 年-月-日-时-分-秒-毫秒 的形式，
